...
- `src/lib/spark/deviceProfile.ts` — Named device profiles (`desktop` | `mobile`), detection (`detectProfileName()`), complete 22-field global baselines (`getGlobalBaseline()`), effective settings merge (`computeEffectiveSettings()`), minimal diff generation (`computeOverrides()`), and `getDeviceProfile()` for renderer construction (derives its 8 `sparkRenderer` fields from the canonical `GLOBAL_BASELINES` table to avoid duplication). Cone angles are full-width **degrees** (Spark 2.1 API: default `coneFov0: 90`, `coneFov: 120`).
...
**API:**
- `attach(controls: SparkControls, profileName?: DeviceProfileName): () => void` — Register a SparkControls as the active controller with its device profile name. Returns a `detach()` function. If a controller was already active, it is replaced and subscribers are notified.
