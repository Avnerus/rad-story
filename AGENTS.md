...
- `src/lib/studio/scroll-animator/transactionGuard.ts` — Suppresses source sync for ScrollAnimator transforms (only `keyframes` and `showChildCameraFrustumWhenSelected` persist) and SparkControls transforms. For SparkControls, uses identity-aware `activeSparkControlsRuntime.canSourceSync(tx.object)` to check if the exact transaction controller is the current active persistable controller. Only exact-root `profileSettings` is allowed for active persistable controllers. Non-persistable and stale/detached controllers have ALL Spark source sync blocked. Uses narrow structural types (no private imports).
...
- `src/lib/studio/spark-controls/activeSparkControlsRuntime.ts` — `ActiveSparkControlsRuntime` class and `activeSparkControlsRuntime` singleton. Reactive registry for the active scene's `SparkControls` instance, device profile name, and source sync capability. `attach(controls, profileName, { sourceSyncEnabled })` returns an identity-safe `detach()` function. `canSourceSync(controls)` checks identity-aware source sync permission for a specific controller. `profileName` getter returns active profile. `sourceSyncEnabled` getter returns current capability. `onChange(fn)` subscribes to active controller changes (including same-controller metadata changes). Used by `SceneRuntime` (attach on mount, detach on destroy), `SparkControlsExtension` (react to active controller), and `transactionGuard` (identity-aware sync check). See "Active Spark Controls Runtime" section below.
- `src/lib/spark/sparkSettingsToRendererOptions.ts` — Pure helper: converts a complete `SparkSettings` snapshot to `SparkRendererOptions`. Maps `lodSplatCount: null` to `undefined`. No baseline literals — all values from the snapshot.
...
- `src/lib/spark/deviceProfile.ts` — Named device profiles (`desktop` | `mobile`), detection (`detectProfileName()`), complete 22-field global baselines (`getGlobalBaseline()`), effective settings merge (`computeEffectiveSettings()`), minimal diff generation (`computeOverrides()`), and `getDeviceProfile()` returning only `profileName` and `dpr` (no Spark settings — those come from `getGlobalBaseline()`). Cone angles are full-width **degrees** (Spark 2.1 API: default `coneFov0: 90`, `coneFov: 120`).
...
**Ad-hoc query parameter**: The landing/ad-hoc viewer uses `?splat_url=<RAD URL>` (not `?url=`). The `splat_url` parameter pre-fills the landing input and is written by the Start button. The legacy `url` parameter is removed if present. Unrelated parameters like `debug` are preserved.
...
**Authoritative `profileSettings` property:** `SparkControls` has a typed, copy-returning `profileSettings` getter and writable setter. Both the constructor and setter route active-profile override values through the private `computeValidatedSettings()` method, which validates each override through the canonical `validateField()` path (clamping, page rounding, NaN/Infinity fallback), applies coupled invariants (`coneFov0 <= coneFov`, `minPixelRadius <= maxPixelRadius`), and merges with the stored baseline. The setter then emits one coherent change notification. This makes `profileSettings` the authoritative target for source sync and undo/redo — transaction commit, undo, and redo all update both the nested overrides and live effective controller settings. Defensive copies prevent caller mutation after assignment. Persisted out-of-range/NaN/Infinity/page-size/coupled values are always clamped to the canonical validation contract.
...
**Source sync:** The `<T is={sparkControls} name="Spark" profileSettings={{ desktop: {}, mobile: {} }} />` literal is the scene-local source of truth. Studio rewrites this attribute via the `profileSettings` setter on the SparkControls instance. The setter merges overrides with the stored baseline and updates effective settings. Works in both playback and edit modes after a cold reload. **File-backed scenes** (`/scene/{name}/edit`) register with `sourceSyncEnabled: true` — the transaction guard's `canSourceSync(controls)` identity check allows exact-root `profileSettings` sync for the active persistable controller. **Ad-hoc viewer** (`RadStoryScene`) registers with `sourceSyncEnabled: false` — ALL Spark source sync is blocked at both the pane (no transaction committed) and the guard (`canSourceSync` returns false). The ad-hoc pane shows a "Session-only" warning. Stale/detached controllers never inherit a newer controller's permission. Transforms, legacy `settings`, individual field names, and nested paths like `profileSettings.desktop.blurAmount` remain blocked everywhere.

**Override rules:**
- Persist only deltas (fields differing from global baseline).
- Use own-property presence (not truthiness) to distinguish "no override" from valid falsey values (`false`, `0`, `null`).
...
**Single profile detection:** `profile.profileName` is the single source of truth for profile detection, set once at app startup by `getDeviceProfile()`. Scenes use `profile.profileName` — no independent `detectProfileName()` calls in scene files.
...
**API:**
- `attach(controls: SparkControls, profileName?: DeviceProfileName, options?: { sourceSyncEnabled?: boolean }): () => void` — Register a SparkControls as the active controller with its device profile name and source sync capability. Returns a `detach()` function. If a controller was already active, it is replaced and subscribers are notified. File-backed edit mode uses `{ sourceSyncEnabled: true }`; ad-hoc viewer uses `{ sourceSyncEnabled: false }`.
...
- `sourceSyncEnabled` getter — Whether source sync is enabled for the active controller. `true` for file-backed edit mode, `false` for ad-hoc viewer.

**Lifecycle:**
1. `SceneRuntime.onMount()` calls `activeSparkControlsRuntime.attach(sparkControls, sparkControls.profileName, { sourceSyncEnabled })` and stores the returned `detach`.
